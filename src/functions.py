...
class Node:
    # Contaisn the Information of the node and another nodes of the Decision Tree.
    def __init__(self):
        self.value= None 
        self.next= None 
        self.childs= None
...
class DecisionTreeClassifier:
    ...
    def _get_entropy():
        ...
        Parametros:
        x_ids: list, list containing the instances of ID's
        Return:
    ...
    def _get_information_gain(self, x_ids, feature_id):
        """
        Calculates the information gain for a given feature based on its entropy and the local entropy of the system
        Parameters:
        x_ids: list, List containing the instances ID's
        feature_id: int, Feature ID
        Return:
        INFO_GAIN: FLOAT, The information gain for a given feature.
        """

        # Calculate total entropy
        info_gain= self._get_entropy(x_ids)

        # Store in a list all the values of the Chosen 
        ...
        info_gain= info_gain- sum([val_counts/ len(x_ids)* self._get_entropy(vald_ids)
        for val_counts, val_ids in zip(feature_vals_count, feature_vals_id)])

        return info_gain


    def _get_feature_max_information_gain(self, x_ids: list, feature_ids: list):
        """
        Finds the attribute feature that maximizes the information gain.
        Parameters:
        x_ids: list, List containing the samples IDs
        feature_ids: List, list contaning the feature ID's
        Returns:
        string and int, feature and feature id of the feature that maximizes the information gain
        """
    ...
    def _id3_recv(self, x_ids, feature_ids, node):
        ...
        Parameters:
        x_ids: list, list containing the sampes ID's
        feature_ids: list, List containing the feature ID's
        node: object, an An instance of the class Nodes
        Return:
        An instance of the classs Node containing all the information of the nodes in the Decision Tree
        """
        if not node:
            node= Node()
        ...
        # Ifa all the example have the same class (pure node) return node
        ...
        for value in feature_values:
            child= Node()
            child.value= value # Add a branch fro mthe node to each feature value in our feature
            node.childs.append(child)
            child_x_ids= [x for x in x_ids if self.x[x][best_feature_id]== value]
            if not child_x_ids:
                child.next= max(set(labels_in_features), keyOlabels_in_features.count)
                print('')
            else:
